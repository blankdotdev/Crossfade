...
abstract class AppDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        private val MIGRATION_5_6 = object : Migration(5, 6) {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    "ALTER TABLE history_table ADD COLUMN isPodcast INTEGER NOT NULL DEFAULT 0"
                )
                database.execSQL(
                    "ALTER TABLE history_table ADD COLUMN isPodcastEpisode INTEGER NOT NULL DEFAULT 0"
                )
            }
        }
